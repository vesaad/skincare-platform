...
function chooseFallbackRoutine(payload) {
  ...
  const [routine, score] = scores.sort(
    (a, b) => b[1] - a[1]
  )[0];
  ...
  if (payload.Skin_Type === 'Sensitive') {
    return 'Sensitive Skin Routine';
  }

  if (payload.Skin_Type === 'Dry') {
    return 'Hydration Routine';
  }

  if (payload.Skin_Type === 'Oily') {
    return 'Acne Routine';
  }
...
function buildFallbackProducts(routine) {
  ...
  return FALLBACK_STEPS[routine]
    .map(([category, ingredient]) => {
    ...
    data: {
      ...
      routineSteps: {
        create: data.products.map((product, index) => ({
          ...
          timeOfDay: 'Daily',
          instructions:
            product.step || product.category || null,
        })),
      },
    },
...
const assess = async (userId, inputs) => {
  try {
    const mlPayload = toMlPayload(inputs);
    ...
    try {
      ...
      console.warn(
        'ML service unavailable, using fallback assessment:',
        err.message
      );
